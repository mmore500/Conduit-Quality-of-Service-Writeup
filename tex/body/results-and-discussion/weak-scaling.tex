...
For the second variable, we tested with \numprint{2048} simulation units (``sim units'') per processor (consistent with the benchmarking experiments performed in Sections \ref{sec:multiprocess-benchmarks} and \ref{sec:multithread-benchmarks}) and just one simulation unit per processor.
...
Across process counts, update period registers around \SI{80}{\micro\second} with one sim unit per CPU and around \SI{200}{\micro\second} with \numprint{2048} sim units per CPU.
...
% Outlier observations range up to around 10ms with \numprint{2048} sim units per CPU and up to slightly less than 100ms inlet / 4s outlet with 1 sim unit.
...
Between 64 and 256 processes, mean update period actually decreased significantly for runs with \numprint{2048} sim units per CPU.
...
Walltime latency sits at around \SI{500}{\micro\second} for one-sim-unit runs and around 2ms for \numprint{2048}-sim-unit runs.
...
In 256 process, \numprint{2048}-sim-unit, one CPU per node runs, outliers of more than 10s occur.
...
Updates latency sits around 7 updates for runs with one sim unit per CPU and around 1.2 updates for runs with \numprint{2048} sim units per CPU.
...
The \numprint{2048} sim units per CPU, one CPU per node, 256 process condition exhibited outliers of up to almost \numprint{8000} update updates latency.
...
Over 16, 64, and 256 processes, mean updates latency increased with process count only under 1 CPU per node, \numprint{2048} sim unit per CPU conditions.
...
On \numprint{2048}-sim-unit-per-CPU runs, median delivery bunching was lower at around 0.4.
...
In fact, over 16, 64, and 256 processes bunching significantly decreased with process count in all conditions except four CPUs per node with \numprint{2048} sim units per CPU.
