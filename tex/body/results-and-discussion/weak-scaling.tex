...
Across process counts, update period registers around \SI{80}{\micro\second} with one simel per CPU and around \SI{200}{\micro\second} with \numprint{2048} simels per CPU.
...
In all cases except one simel per CPU with four CPUs per node, mean update period increased significantly with processor count from 16 to 64 to 256 CPUs.
However, from 64 to 256 processors mean update period only increased significantly with one simel per CPU per node.
...
Except for the extreme case of one simel per CPU and one CPU per node, update period quality of service is stable in scaling from 64 to 256 processes.
...
Updates latency sits around 7 updates for runs with one simel per CPU and around 1.2 updates for runs with \numprint{2048} simels per CPU.
...
On \numprint{2048}-simel-per-CPU runs, median delivery bunching was lower at around 0.4.
